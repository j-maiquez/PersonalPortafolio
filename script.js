...
next.addEventListener("click", event => {//go to next card
    width = projects.offsetWidth //recalculate
    projects.scrollBy(width + gap, 0)
    console.log(width)
})

bext.addEventListener("click", event => {//go to prev card
    width = projects.offsetWidth //recalculate
    projects.scrollBy(-(width + gap), 0)
})

//Selecting the elements
const drawerItems = document.querySelectorAll (" .infowrapper")
...
drawerItems.forEach(item => {
    ...
    question.addEventListener("click", () => {
        ...
        drawerAnswers.forEach(answer => (answer.classList.remove))

        //for this drawer we've clicked, open fit if not already
        if(!open) {
